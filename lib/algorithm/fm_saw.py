...
# taken from https://github.com/ZJUJeffLai/SAW_SSL/

class FixMatchSAW(SemiSupervised):

    def __init__(self, cfg: CfgNode) -> None:
        super().__init__(cfg)
        
        # original SAW update the p_model after one epoch 
        self.m = 0.999
    ...
    def run_step(self) -> None:
        ...
        # change loss_weight here
        # multiply the weight with estimated learning difficulty
        # which is the inversed class frequency
        # "effective" + "normalization"
        # I need get the pseudo-label from the whole training epoch.
        if not self.p_model.is_cuda:
            self.p_model = self.p_model.to(self.device)
        self.update(p)

        distb = self.p_model * len(self._ul_iter)

        N = torch.sum(distb) / len(distb)
        beta = (N - 1) / N
        
        ones = torch.ones(distb.shape).cuda()
        ...
        loss_weight = loss_weight * class_weight[max_idx]
        # end here
